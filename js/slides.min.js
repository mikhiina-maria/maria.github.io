function checkNumber(number) {
  ...
  if (number === 2) {
    return $('[data-name="catalog"]');
  }
  if (number === 3) {
    return $('[data-name="service"]');
  }
  if (number === 4) {
    return $('[data-name="hotItWork"]');
  }
  if (number === 5) {
    return $('[data-name="contact"]');
  }
  if (number === 6) {
    return $('[data-name="download"]');
  }
}

function checkSlides(body) {
  let stage;
  const regexp = /(?<=\-{1})\d+?/i;

  Object.keys(body.classList).forEach((number) => {
    if (body.classList[number].startsWith("stage")) {
      return (stage = body.classList[number]);
    }
  });
  const slideNumber = +stage.match(regexp)[0];

  return checkNumber(slideNumber).data();
}

function manipulateSLides() {
  const $body = document.body;
  const $elementDataset = checkSlides($body);
  const $elementNavigationLink = $(`[data-link="${$elementDataset.name}"]`)[0];

  manipulate($elementNavigationLink);
}
    ...
    $(".slide").each(function (index, element) {
      ...
      if (window.setStageClasses != 0) {
        if (halfOnScreen && allowToSelect) {
          ...
          const $elementDataset = $element.data();
          const $elementNavigationLink = $(
            `[data-link="${$elementDataset.name}"]`
          )[0];
